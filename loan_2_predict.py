    ...
    if st.session_state.trained:
        ...
        if st.session_state.model is not None:
            if st.button('Predict'):
                ...
                prediction_threshold = 0.5  # Threshold can be adjusted based on model calibration
                prediction_label = 'True' if prediction_proba >= prediction_threshold else 'False'
                st.write(f'Risk Flag (Predicted): {prediction_label} (Probability: {prediction_proba:.2f})')
        else:
